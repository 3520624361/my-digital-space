...
const REMOTE_STORAGE_ENDPOINT = window.__SOLARA_STORAGE_BASE || "/api/storage";
...
// API配置 - 修复API地址和请求方式
const API = {
    baseUrl: window.__SOLARA_API_BASE || "/api/proxy",
...
async function fetchPaletteData(imageUrl, signal) {
    ...
    const response = await fetch(`${window.__SOLARA_PALETTE_BASE || "/api/palette"}?image=${encodeURIComponent(imageUrl)}`, { signal });
